...
/**
 * Copy of Jersey's Multipart Writer.
 * 
 * This writer was modified to fix boundary content length.
 * 
 * Do not use in prod!!!
 * 
*/
...
@Singleton
@Produces("multipart/*")
public class CustomMultipart implements MessageBodyWriter<MultiPart> {
   ...
   public CustomMultipart(@Context final Providers providers) {
   ...
   public void writeTo(final MultiPart entity,
       ...
       final MediaType boundaryMediaType = generateBoundary(mediaType); //Boundary.addBoundary(mediaType);
       if (boundaryMediaType != mediaType) {
           headers.putSingle(HttpHeaders.CONTENT_TYPE, boundaryMediaType.toString());
       }

       final String boundaryString = boundaryMediaType.getParameters().get(Boundary.BOUNDARY_PARAMETER); //generateBoundary();
   ...
   public static MediaType generateBoundary(MediaType mediaType) {
       final Map<String, String> parameters = new HashMap<String, String>(
               mediaType.getParameters());
       parameters.put(Boundary.BOUNDARY_PARAMETER, generateBoundary());
